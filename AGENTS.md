...
- **v4 skill migration template (canonical recipe).** New v4 skills follow this exact shape, proven across 3 skill types (reasoning, CRUD, free-form capture): single tool with array `writes` argument; handler builds an `ActionPlan` with defensive defaults for the data type's required fields; lazy-imports `executor.applyWrites` inside try/catch (FEAT057 B1 pattern); returns `{ success, userMessage, items?, clarificationRequired, data }`. Manifest `dataSchemas.write` matches the file key the handler writes to. `context.ts` declares only keys in `SUPPORTED_KEYS` — extend the resolver in `skillDispatcher.ts` only when truly new context is needed. No chat.tsx changes for new skills (FEAT056 + FEAT057 already wired the consumer paths). (CR-FEAT058)
